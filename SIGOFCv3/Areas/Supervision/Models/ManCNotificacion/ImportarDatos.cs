...
namespace SIGOFCv3.Areas.Supervision.Models.ManCNotificacion
{
    ...
    {
        public static List<CapaEntidad.DOC.Ent_ISUPERVISION_DET_EMADERABLE> MuestraMaderable(string asCodCNotificacion, HttpRequestBase _request)
        {
            CapaEntidad.DOC.Ent_ISUPERVISION_DET_EMADERABLE paramsCN = new CapaEntidad.DOC.Ent_ISUPERVISION_DET_EMADERABLE();
            CapaLogica.DOC.Log_CNOTIFICACION exeCN = new CapaLogica.DOC.Log_CNOTIFICACION();
            ...
            if ((file != null) && (file.ContentLength > 0) && !string.IsNullOrEmpty(file.FileName))
            {
                using (var package = new ExcelPackage(file.InputStream))
                {
                    ...
                    for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                    {
                        ...
                        codSistema = workSheet.Cells[rowIterator, 19].Value.ToString().Trim().Split('|');
        ...
        public static List<CapaEntidad.DOC.Ent_ISUPERVISION_DET_EMADERABLE> MuestraNoMaderable(string asCodCNotificacion, HttpRequestBase _request)
        {
            CapaEntidad.DOC.Ent_ISUPERVISION_DET_EMADERABLE paramsCN = new CapaEntidad.DOC.Ent_ISUPERVISION_DET_EMADERABLE();
            CapaLogica.DOC.Log_CNOTIFICACION exeCN = new CapaLogica.DOC.Log_CNOTIFICACION();
            List<CapaEntidad.DOC.Ent_ISUPERVISION_DET_EMADERABLE> lstCenso = new List<CapaEntidad.DOC.Ent_ISUPERVISION_DET_EMADERABLE>();
