...
using CEntidad = CapaEntidad.DOC.Ent_SOLEXT;
...
namespace CapaDatos.DOC
{
    ...
    public class Dat_SOLEXT
    {
        ...
        /// <returns></returns>
        public List<CEntidad> RegMostrarSOLEXT_Buscar(SqlConnection cn, CEntidad oCEntidad)
        {
        ...
        /// <param name="oCEntidad"></param>
        /// <returns></returns>
        public String RegGrabaSolicitudExterna(SqlConnection cn, CEntidad oCEntidad)
        {
            ...
            try
            {
                ...
                //Grabando Cabecera
                using (SqlCommand cmd = oGDataSQL.ManExecuteOutput(cn, tr, "DOC.spSOLEXTGrabar", oCEntidad))
                {
                    cmd.ExecuteNonQuery();
                    ...
                    else if (OUTPUTPARAM01 == "2")
                    {
                        tr.Rollback();
                        tr = null;
                        throw new Exception("No Tiene Permisos para Modificar esta solicitud de información externa");
                    }
                ...
                {
                    oCEntidad.COD_SOLEXT = OUTPUTPARAM01;
                }
                //  Eliminando Detalle               
                if (oCEntidad.ListEliTABLA != null)
                {
                    ...
                    {
                        oCamposDet = new CEntidad();
                        oCamposDet.COD_SOLEXT = OUTPUTPARAM01;
                        ...
                        oGDataSQL.ManExecute(cn, tr, "DOC.spSOLEXTEliminarDetalle", oCamposDet);
                    }
                }
                    ...
                    if (oCEntidad.ListInformes != null)
                    {
                        foreach (var loDatos in oCEntidad.ListInformes)
                        {
                            if (loDatos.RegEstado == 1 || loDatos.RegEstado == 2) //Nuevo o Modificado
                            {
                                ...
                                oGDataSQL.ManExecute(cn, tr, "DOC.spSOLEXT_DET_INFORME_Grabar", oCamposDet);
                            }
                        }
                    }
                }
                    ...
                    if (oCEntidad.ListInformes != null)
                    {
                        foreach (var loDatos in oCEntidad.ListInformes)
                        {
                            if (loDatos.RegEstado == 1 || loDatos.RegEstado == 2) //Nuevo o Modificado
                            {
                                ...
                                oCamposDet.COD_SOLEXT = OUTPUTPARAM01;
                                oCamposDet.RegEstado = 1;
                                oGDataSQL.ManExecute(cn, tr, "DOC.spSOLEXT_DET_THABILITANTE_Grabar", oCamposDet);
                            }
                        }
                    }
                }

                if (oCEntidad.TIPO == "EX")
                {
                    ...
                    if (oCEntidad.ListInformes != null)
                    {
                        foreach (var loDatos in oCEntidad.ListInformes)
                        {
                            if (loDatos.RegEstado == 1 || loDatos.RegEstado == 2) //Nuevo o Modificado
                            {
                                ...
                                oCamposDet.COD_RESODIREC_INI_PAU = loDatos.COD_RESODIREC_INI_PAU;
                                oCamposDet.RegEstado = 1;
                                oGDataSQL.ManExecute(cn, tr, "DOC.spSOLEXT_DET_EXP_Grabar", oCamposDet);
                            }
                        }
                    }
                }
        ...
        /// <param name="oCEntidad"></param>
        /// <returns></returns>
        public CEntidad RegMostrarListaSolExtItem(SqlConnection cn, CEntidad oCEntidad)
        {
            CEntidad lsCEntidad = new CEntidad();
            try
            {
                using (SqlDataReader dr = oGDataSQL.SelDrdDefault(cn, null, "DOC.spSOLEXTMostrarItems", oCEntidad))
                {
                    if (dr != null)
                    {
                        ...
                        {
                            dr.Read();
                            lsCEntidad.COD_SOLEXT = dr.GetString(dr.GetOrdinal("COD_SOLEXT"));
                            lsCEntidad.TIPO_FISCALIZA = dr.GetString(dr.GetOrdinal("TIPO_FISCALIZA"));
                            lsCEntidad.NUMERO_SOLEXT = dr.GetString(dr.GetOrdinal("NUMERO_SOLEXT"));
                            lsCEntidad.COD_TIPO_DOCUMENTO = dr.GetString(dr.GetOrdinal("COD_TIPO_DOCUMENTO"));
                            lsCEntidad.COD_FSOLICITA = dr.GetString(dr.GetOrdinal("COD_FSOLICITA"));
                            lsCEntidad.FSOLICITA = dr.GetString(dr.GetOrdinal("FSOLICITA"));
                            lsCEntidad.TIPO = dr.GetString(dr.GetOrdinal("TIPO"));
                            lsCEntidad.FECHA_EMISION = dr.GetString(dr.GetOrdinal("FECHA_EMISION"));
                            lsCEntidad.COD_FDIRIGE = dr.GetString(dr.GetOrdinal("COD_FDIRIGE"));
                            lsCEntidad.FDIRIGE = dr.GetString(dr.GetOrdinal("FDIRIGE"));
                        ...
                        if (lsCEntidad.TIPO == "EX")
                        {
                            ...
                            if (dr.HasRows)
                            {
                                ...
                                while (dr.Read())
                                {
                                    ...
                                    ocampoEnt2.D_LINEA = dr.GetString(pt3);
