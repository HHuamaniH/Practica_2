...
namespace SIGOFCv3.Areas.Tribunal.Models.TribunalNotificacion2da
{
    public class ExportarDatos
    {
        public static ListResult RegistroUsuario(string asCodUsuario)
        {
            ...
            try
            {
                CEntidad2da paramCap = new CEntidad2da();
                CLogica2da exeCap = new CLogica2da();
                paramCap.TIPO = "REGISTRO_USUARIO";
                paramCap.COD_UCUENTA = asCodUsuario;

                List<Dictionary<string, string>> olResult = exeCap.ReporteFiscalizacionNotificacion2da(paramCap);
                    ...
                    using (OleDbConnection conn = new OleDbConnection(cb.ConnectionString))
                    {
                        ...
                        using (OleDbCommand cmd = conn.CreateCommand())
                        {
                            ...
                            foreach (var itemPart in olResult)
                            {
                                ...
                                insertar = insertar + ",'" + (itemPart["RESOLUCION_TFFS"] ?? "") + "'";
