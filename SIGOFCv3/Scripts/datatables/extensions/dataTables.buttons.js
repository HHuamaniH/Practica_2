...
var _exportData = function ( dt, inOpts )
{
    ...
    /* ----- BEGIN changed Code ----- */
    //08062018//var headerMatrix = getHeaders(dt.settings()[0]);
	...
	var columnsExp = [];
	if (config.columns!="") {
	    for (var i = 0; i < rawHeaderMatrix[0].length; i++) {
	        if (!config.columns.includes(i)) {
	            columnsExp.push(i);
	        }
	    }
	} else {
	...
	for (var rowIdx = 0; rowIdx < numRows; rowIdx++) {
	    var headerRow = dt.columns(columnsExp).indexes(':visible').map(function (colIdx) {
    ...
    */
	...
	var footer = dt.table().footer() ?
		dt.columns(columnsExp).indexes().map(function (idx) {
	...
	var selectedCells = dt.cells(rowIndexes, columnsExp);
	...
	return {
	    //header: header, /* ----- BEGIN changed Code ----- */
	    header: headerMatrix,
		footer: footer,
		body:   body
	};
...
/* ----- BEGIN added Code -----https://datatables.net/forums/discussion/comment/106240? */
var getHeaders = function (dt) {
    var thRows = dt.table().header().rows; //var thRows = dt.nTHead.rows;
