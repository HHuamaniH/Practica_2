    ...
    data_extend = [
        {
            "data": "ESTADO_PROGRAMA", "title": "Opción", "width": "7%", "orderable": false, "searchable": false, "mRender": function (data, type, row, meta) {
                var selectNo = (data == true) ? "" : "selected";
                var selectSi = (data == true) ? "selected" : "";
                return getSelect(row, selectNo, selectSi)
            }
        },
        {
            "data": "OBSERVACION", "title": "", "width": "65%", "orderable": false, "searchable": false, "mRender": function (data, type, row, meta) {
                return getInputTwo(row)
            }
        }
    ];
    options = { page_length: 10, row_index: true, data_extend: data_extend, page_autowidth: false };
...
ManInforme_AddEditFauna.fnGetListPrograma = function (_tipo) {
    ...
    if (dt.$("tr").length > 0) {
        $.each(dt.$("tr"), function (i, o) {
            ...
            } else if (_tipo == "APROVECHAMIENTO_SOSTENIBLE") {
                data["OBSERVACION2"] = $($($(dataHtml).find("td")[3]).find("input")[1]).val();
            }
...
ManInforme_AddEditFauna.fnSaveForm = function () {
    ...
    $.ajax({
        ...
        complete: function () {
            ...
            datosInforme.tbAprovSostenible = _renderAprovSostenible.fnGetList();
            datosInforme.tbEliTABLA = datosInforme.tbEliTABLA.concat(_renderAprovSostenible.fnGetListEliTABLA());
...
function getInputTwo(row) {
    ...
    switch (row.COD_PROGRAMA) {
        case 55:
            ...
            html += 'Éxito Reproductivo<input class="form-control form-control-sm" type="text" value="' + row.OBSERVACION2 + '" style="width:100%;"/>';
            break;
        case 56:
            html = 'Actividades<input class="form-control form-control-sm" type="text" value="' + row.OBSERVACION + '" style="width:100%;"/> <br/>';
