...
namespace SIGOFCv3.Helper
{
    public class EpplusExcel
    {
        ...
        {
            //Condición añadida para que solo pinte cuando se trate de un texto no vácio.
        ...
        public void setHeader(ExcelWorksheet _sheet, string celda, string texto, Boolean mergue)
        {
            ...
            _sheet.Cells[celda].Style.Fill.BackgroundColor.SetColor(Color.FromArgb(189, 215, 238));
        }
        ...
        public int pintarcabecerasInformeSupervision(List<string> _cabecera, List<CapaEntidad.ViewModel.DOC.VM_FControlCalidadSupervision_Det> listadoitems, ExcelWorksheet _Sheet, string header)
        {
            int finish = -1;
            foreach (var x in _cabecera)
            {
            ...
            foreach (var x in listadoitems)
            {
                finish++;
                string letra = convertNumberToLetter(finish);
                setHeader(_Sheet, letra + "2:" + letra + "2", x.codigo, false);
                setHeader(_Sheet, letra + "3:" + letra + "3", "S - " + x.hijo, false);
                finish++;
                letra = convertNumberToLetter(finish);
                setHeader(_Sheet, letra + "2:" + letra + "2", x.codigo, false);
                setHeader(_Sheet, letra + "3:" + letra + "3", "N - " + x.hijo, false);
                finish++;
                letra = convertNumberToLetter(finish);
                setHeader(_Sheet, letra + "2:" + letra + "2", x.codigo, false);
                setHeader(_Sheet, letra + "3:" + letra + "3", "DETALLE - " + x.hijo, false);
            }
            _texto_sin_borde_Titulo(_Sheet, convertNumberToLetter(0) + "1:" + convertNumberToLetter(finish) + "1", header, Color.White, Color.Black, "Calibri");
            return finish;
        }
